...
func TestWidthValues(t *testing.T) {
	if Width.Sidebar != 18 {
		t.Errorf("Width.Sidebar = %d, want 18", Width.Sidebar)
	}
	if Width.Label != 12 {
...
// TestTypographyFactories verifies typography functions return valid styles.
func TestTypographyFactories(t *testing.T) {
	p := Dark

	// Each factory should return a non-nil style that can render
	factories := []struct {
		name string
		fn   func(Palette) lipgloss.Style
	}{
	...
	for _, f := range factories {
		t.Run(f.name, func(t *testing.T) {
			...
			if result == "" {
				t.Errorf("%s returned empty render", f.name)
			}
		})
	}
}
